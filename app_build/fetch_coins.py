...
# Constants
BUILD_CONFIG_PATH = "app_build/build_config.json"
COINS_CI_ASSET_PATH = "assets/config/coins_ci.json"
...
class CoinCIConfig:
    ...
    @staticmethod
    def load(path: str = BUILD_CONFIG_PATH):
        print(f"Loading coins config from {path}")
        with open(path, "r") as f:
            data = json.load(f)
            if "coins" not in data:
                print("Failed to load build_config.json")
                exit(1)
            return CoinCIConfig(**data["coins"])  # type: ignore

    def save(
        self,
        asset_path: str = COINS_CI_ASSET_PATH,
        ...
        # read the existing build_config.json file
        # update the coins section with the latest commit hash
        print(f"Saving coins config to {asset_path}")
        data: Dict[str, Any] = {}
        with open(asset_path, "w+") as f:
            json.dump(self.__dict__, f, indent=4)

        if update_build_config:
            print(f"Saving coins config to {build_config_path}")
            with open(build_config_path, "r") as f:
                data = json.load(f)
                data["coins"] = self.__dict__
