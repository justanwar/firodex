...
void testCalculateBuyAmount() {
  test('Calculation sellAmount on selectedOrder', () {
    ...
    expect(
      calculateBuyAmount(
          sellAmount: Rational.fromInt(2), selectedOrder: bestOrder),
      Rational.fromInt(4),
    );
    expect(
      calculateBuyAmount(
          sellAmount: Rational.parse('0.1'), selectedOrder: bestOrder),
      Rational.parse('0.2'),
    );
    expect(
      calculateBuyAmount(
          sellAmount: Rational.parse('1e-30'), selectedOrder: bestOrder),
    ...
    expect(
      calculateBuyAmount(
          sellAmount: Rational.parse('1e-30'), selectedOrder: bestOrder2),
      Rational.parse('1e-60'),
    );
    expect(
      calculateBuyAmount(
          sellAmount: Rational.parse('1e70'), selectedOrder: bestOrder2),
      Rational.parse('1e40'),
    );
    expect(
      calculateBuyAmount(
  ...
  test('Negative tests', () {
    ...
    expect(
        calculateBuyAmount(
            sellAmount: Rational.fromInt(2), selectedOrder: null),
        isNull);
    expect(
        calculateBuyAmount(sellAmount: null, selectedOrder: bestOrder), isNull);
  });
}
